...
// Create redis client
var redis = new Redis(config.get('redis')).client();
...
function Api(id, key, options) {
  ...
  var self = this;
  redis.get(prefix + id, function (err, value) {
    if (err) {
      return;
    }
...
Api.prototype.generate = function (callback) {
  ...
  this._request(options, function (err) {
    ...
    redis.set(prefix + self.credentials.id, key, function (err) {
      return callback(err);
    });
  });
};
...
Api.prototype.hawk = function (callback) {
  var self = this;
  redis.get(prefix + this.credentials.id, function (err, value) {
    if (err) {
      return callback(err);
    }
